...
#     def __init__(self, d_model, num_heads, d_ffn, drop_prob, num_blocks, d_out, max_sequence_length):
#         super().__init__()
...
#         #print("Before embedding: ", x.shape, y.shape)
#         x = x.long()
#         y = y.long()
#         x = self.embedding(x)
#         y = self.embedding(y)
        
#         #print("After embedding: ", x.shape, y.shape)
#         # 确保位置编码前数据形状为 [batch_size, max_seq_len, d_model]

#         x = self.positional_encoding(x)
#         y = self.positional_encoding(y)
#         #print("After positional encoding: ", x.shape, y.shape)
...
#         # 全局平均池化，将 [batch_size, max_seq_len, num_classes] 转换为 [batch_size, num_classes]
#         y = y.permute(0, 2, 1)  # 将维度从 [batch_size, 784, num_classes] 变为 [batch_size, num_classes, 784]
#         y = self.global_avg_pool(y)  # 全局平均池化
#         y = y.squeeze(2)  # 去掉最后一个维度，变为 [batch_size, num_classes]
